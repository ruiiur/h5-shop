...
<template>
    <div class="pay-coupon">
        ...
        <div class="order-info">
            <h3>可抵金额：{{deductionamt}}元X{{couponNum}}</h3>
...
import { finProOtherProduct,postCouponsPay,uploadPayImg,getBankAccount,getOrderInfoForPay } from '../services/index'
            ...
            if(this.payState==1){
                ...
                postCouponsPay(params).then(response => {
                    ...
                    if (response.meta.success == true) {
                        ...
                        this.showTip=true;//弹框是否显示
                        this.tipText='支付成功';//弹框内容
                        setTimeout(()=>{ this.$router.go(-1); }, 3000);
                    } else {
                        console.log("优惠券购买提交失败");
                        this.showTip=true;//弹框是否显示
                        this.tipText=response.meta.message;//弹框内容
                        this.payState=1;
                    }
                })
                .catch(err => {
